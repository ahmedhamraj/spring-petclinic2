    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                ssh -i ${PEM_KEY} -o StrictHostKeyChecking=no ${EC2_USER}@${EC2_HOST} 'pkill -f spring-petclinic || true'

                # Use rsync for faster deployment
                rsync -avz -e "ssh -i ${PEM_KEY} -o StrictHostKeyChecking=no" target/${JAR_NAME} ${EC2_USER}@${EC2_HOST}:/home/ubuntu/

                # Start jar in background and detach SSH
